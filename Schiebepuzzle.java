...
public class Schiebepuzzle {
  ...
  /**
   * Default constructor initializes feld as an 2-d array of length 4x4.
   * Fills the array with the perfect solution to ensure any derivation of this field remains solvable.
   */
  public Schiebepuzzle() {
    ...
    this.highestInt = (int) Math.pow(feld.length, 2) -1; // Highest number included in feld,
                                                         // assuming its square and the values are assigned consecutively. 
    this.feld = feld;
  }
  ...
  public String toString() {
    ...
    int maxDLen = String.valueOf(highestInt).length(); // Determines the maximum length (as in String.length()) for all values in feld.
    for (int i = 0; i < this.feld.length; i++) { // Iterates over the row indexes
      sb.append("|"); // Every String line begins with "|"
      for (int j = 0; j < this.feld[i].length; j++) { // Iterates over the column indexes of each row, writes a single cell for column.
        sb.append(String.format(feld[i][j] != 0 ? "%"+maxDLen+"d" : "  ", feld[i][j]));
        sb.append("|");
      }
      sb.append("\n");
      sb.append("-".repeat(feld[i].length * 3 + 1)); // Interjects a line between each row and at the end.
      sb.append("\n");
    }
    return sb.toString();
  }
  ...
  /**
   * Tries to move a specified number into the empty slot.
   * Will throw a RuntimException if the number is out of Range or the slot with the number is not a direct neighbor of the empty slot.
   * 
   ...
   * @return True if the value is in the range between the highest and lowest value slot, else returns false, 
   */
  ...
  /**
   * Determines the position of any given number.
   * Throws a RuntimeException if the number is not in range of feld.
   * 
   * @param i Value of the sought slot.
   * @return Array of length 2, index 0 is the row, index 1 is the column number. Both indexes are -1 if i is not contained in feld.
   */
  public int[] getPosition(int i) {
     ...
     * As the validity of i is checked beforehand, this iteration should be successfull.
     * Otherwise the Returnvalue is {-1, -1}.
     */
  ...
  /**
   * Determines if the given slot is next to the empty slot and thus can be moved into the empty slot.
   * Slots cannot be moved diagonally!
   * 
  ...
  public boolean istVerschiebbar(int i) {
    ...
    int maxC = feld[0].length - 1; // Determines the highest index for columns (assuming a square feld).

    int[] position = getPosition(i);
    int row = position[0];
    int col = position[1];

    if (col != -1 || row != -1) { // Checks validity of getPostion output: Index -1 indicates something is wrong with i.
      /*
       * Series of checks covering the horizontal and vertical environment of i.
       * If any of the 4 neighboring fields has the value 0, return true.
       */
      if (row < maxR) {
        if ((feld[row + 1][col] == 0)) { 
          return true;
